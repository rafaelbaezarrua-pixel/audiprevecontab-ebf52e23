-- Migration to update notification recipients for expirations and company events to all users
-- 1. Update handle_empresa_notification to notify all active users
...
BEGIN
    ...
    FOR v_recipient_id IN (
        SELECT user_id FROM public.profiles WHERE ativo = true
    ) LOOP
...
-- 2. Update check_expirations to notify all active users
...
BEGIN
    -- Temporary table to hold unified expiration data
    ...
    FOR r IN SELECT * FROM temp_expirations LOOP
        ...
        IF v_days_left IN (30, 15, 1, 0) THEN
            ...
            -- Avoid duplicate notifications for same document, same day count, same day creation
            IF NOT EXISTS (
                ...
                -- Send to ALL active users
                FOR v_recipient_id IN (SELECT user_id FROM public.profiles WHERE ativo = true) LOOP
                    INSERT INTO public.notification_recipients (notification_id, user_id)
                    VALUES (v_notification_id, v_recipient_id);
                END LOOP;
            END IF;
