...
VALUES 
    ('expiration', 'Vencimentos', 'Notificações de taxas, licenças e certificados próximos ao vencimento.', true),
    ('company_event', 'Empresas', 'Notificações sobre cadastro e situações de empresas.', true)
    ...
    FOR r IN SELECT * FROM temp_expirations LOOP
        ...
        IF v_days_left IN (30, 15, 1, 0) THEN
            ...
            IF NOT EXISTS (
                ...
                FOR v_recipient_id IN (SELECT user_id FROM public.user_roles WHERE role = 'admin') LOOP
                    INSERT INTO public.notification_recipients (notification_id, user_id)
                    VALUES (v_notification_id, v_recipient_id);
                END LOOP;
            END IF;
