    ...
    <div className="space-y-4">
      ...
      {!loading && empresas.length === 0 && (
        <div className="text-center py-20 bg-black/[0.02] dark:bg-white/[0.01] rounded-2xl border border-dashed border-border/10">
          ...
          <p className="text-[12px] font-black uppercase text-muted-foreground/30 tracking-[0.2em]">Nenhuma empresa encontrada</p>
        </div>
      )}

      <div className="bg-black/[0.03] dark:bg-white/[0.02] border border-border/10 rounded-[2.5rem] shadow-inner p-1 pb-4 relative">
        <div className="hidden md:grid grid-cols-[2fr_1.2fr_1fr_1.2fr_1fr_60px] px-6 py-4 text-[10px] font-black uppercase text-muted-foreground/60 mb-0 relative z-20">
          <span>Empresa</span>
          <span className="text-center">Honorário Base</span>
          <span className="text-center">Mês Ref.</span>
          <span className="text-center">Status Mês</span>
              ...
              <div className="md:grid md:grid-cols-[2fr_1.2fr_1fr_1.2fr_1fr_60px] items-center w-full py-1">
                ...
                <div className="hidden md:flex justify-center">
                  {monthRecord ? (
                    <span className={cn(
                      ...
                      {monthRecord.pago ? "PAGO" : monthRecord.status.toUpperCase()}
                    </span>
                  ) : (
                    <span className="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border border-dashed border-border/10 text-muted-foreground/20">
                      PENDENTE
                    </span>
                  )}
                </div>
                ...
                <div className="hidden md:block text-center">
                  <span className={cn(
                    ...
                    monthRecord ? "text-primary" : "text-muted-foreground/30"
                  )}>
                    {monthRecord ? formatCurrency(monthRecord.valor_total) : "—"}
                  </span>
                </div>
                ...
                <div className="max-w-6xl space-y-4 animate-in fade-in slide-in-from-top-1 duration-200">
                  <div className="flex bg-black/5 dark:bg-white/5 p-1 border-b border-border/5 items-center justify-between px-4 -mx-4 -mt-4 mb-4 rounded-t-2xl">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-black text-muted-foreground/30 uppercase">ID: {emp.id.slice(0, 10).toUpperCase()}</span>
                    </div>
                  </div>
                        ...
                        {!mForm && (
                          <div className="flex flex-col sm:flex-row items-end gap-3 bg-black/5 dark:bg-white/5 p-4 rounded-2xl border border-border/10 shadow-inner">
                            <div className="flex-1 w-full space-y-2">
                              <label className="text-[9px] font-bold text-foreground uppercase ml-1 opacity-50">Gerar Novo Honorário</label>
                              <input
                                type="month"
                            ...
                            <table className="w-full border-collapse">
                              <thead>
                                <tr className="bg-black/5 dark:bg-white/5 border-b border-border/10">
                                  <th className="px-5 py-3 text-left text-[10px] font-black uppercase tracking-[0.15em] text-foreground/40">Competência</th>
                                  <th className="px-5 py-3 text-left text-[10px] font-black uppercase tracking-[0.15em] text-foreground/40">Valor Total</th>
                                  <th className="px-5 py-3 text-left text-[10px] font-black uppercase tracking-[0.15em] text-foreground/40">Vencimento</th>
                                  <th className="px-5 py-3 text-center text-[10px] font-black uppercase tracking-[0.15em] text-foreground/40">Status / Pagamento</th>
                                  <th className="px-5 py-3 text-right text-[10px] font-black uppercase tracking-[0.15em] text-foreground/40 pr-8 w-20">...</th>
                                </tr>
                              </thead>
                                    ...
                                    <td className="px-5 py-3 text-center">
                                      <div className="flex items-center justify-center gap-2">
                                        ...
                                        {record.pago ? (
                                          ...
                                          <span className="px-2.5 py-1 rounded-lg text-[9px] font-black border border-border/10 text-foreground/20 uppercase">ABERTO</span>
                                        )}
                                      </div>
                                    </td>
