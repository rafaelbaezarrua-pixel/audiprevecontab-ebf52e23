...
const DeclaracoesAnuaisPage: React.FC = () => {
    ...
    const filterSocios = () => {
        ...
        return Object.values(groups).filter(p => {
            ...
            if (activeIrpfTab === "entregues") return p.statusIrpf === 'finalizada';
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
          <div className="flex bg-black/10 dark:bg-white/5 p-1 rounded-xl border border-border/10 overflow-x-auto no-scrollbar w-full lg:w-auto shadow-inner gap-1">
            {[{ id: "defis", label: "DEFIS" }, { id: "ecd_ecf", label: "ECD/ECF" }, { id: "dasn_simei", label: "DASN-SIMEI" }, { id: "dirf", label: "DIRF" }, { id: "irpf", label: "IRPF" }].map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab.id ? "bg-card text-primary shadow-sm" : "text-muted-foreground/50 hover:text-foreground"}`}>
                {tab.label}
              </button>
            ))}
          </div>
          ...
          <div className="relative flex-1 w-full lg:max-w-md group">
            ...
            <input type="text" placeholder="BUSCAR EMPRESA OU SÓCIO..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 h-10 bg-black/10 dark:bg-white/5 border border-border/10 rounded-xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-1 focus:ring-primary/20 transition-all placeholder:text-muted-foreground/20 shadow-sm" />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
          <div className="flex items-center gap-2 bg-black/10 dark:bg-white/5 p-1 rounded-xl border border-border/10 w-fit">
            {[{ id: "todos", label: "Todos", color: "text-muted-foreground" }, { id: "pendente", label: "Pendente", color: "text-rose-500" }, { id: "em_andamento", label: "Andamento", color: "text-amber-500" }, { id: "finalizada", label: "Finalizado", color: "text-emerald-500" }].map(st => (
              <button key={st.id} onClick={() => setStatusFilter(st.id)} className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${statusFilter === st.id ? "bg-card shadow-sm " + st.color : "text-muted-foreground/50 hover:text-foreground"}`}>{st.label}</button>
            ))}
          </div>
          ...
          (filteredItems as Empresa[]).map(emp => {
            ...
            return (
              <div key={emp.id} className={`group bg-card border rounded-2xl transition-all duration-200 overflow-hidden shadow-sm ${isExpanded ? 'border-primary/40 ring-1 ring-primary/5' : 'border-border/40 hover:border-primary/20'}`}>
                <div className={`flex items-center justify-between px-4 py-2.5 cursor-pointer transition-colors ${isExpanded ? 'bg-primary/[0.03]' : 'hover:bg-primary/[0.01]'}`} onClick={() => setExpandedEmpresa(isExpanded ? null : emp.id)}>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className={`px-1.5 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest border ${decl.obrigatorio ? "bg-primary/5 text-primary border-primary/20" : "bg-black/5 text-muted-foreground/30 border-border/10"}`}>{decl.obrigatorio ? "OBRIGATÓRIA" : "OPCIONAL"}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-6">
                    ...
                    <div className={`p-1.5 rounded-lg border transition-all ${isExpanded ? 'bg-primary text-white border-primary rotate-180' : 'bg-black/5 dark:bg-white/5 text-muted-foreground/30 border-border/10'}`}><ChevronDown size={12} /></div>
                  </div>
                </div>
                {isExpanded && (
                  <div className="border-t border-border/5 p-4 space-y-4 bg-black/[0.01] dark:bg-white/[0.01] animate-in slide-in-from-top-2 duration-200">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3 bg-black/5 dark:bg-white/5 p-4 rounded-xl border border-border/10 shadow-inner">
                      ...
                      <div className="flex items-end">
                        <button onClick={() => saveDeclaracao(emp.id, activeTab)} disabled={saving} className="w-full h-9 inline-flex items-center justify-center gap-2 rounded-lg bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-widest shadow-md hover:bg-primary/90 transition-all disabled:opacity-50">
          ...
          (filteredItems as UnifiedSocio[]).map(socio => {
            ...
            return (
              <div key={socio.id} className={`group bg-card border rounded-2xl transition-all duration-200 overflow-hidden shadow-sm ${isExpanded ? 'border-primary/40 ring-1 ring-primary/5' : 'border-border/40 hover:border-primary/20'}`}>
                <div className={`flex items-center justify-between px-4 py-2.5 cursor-pointer transition-colors ${isExpanded ? 'bg-primary/[0.03]' : 'hover:bg-primary/[0.01]'}`} onClick={() => setExpandedIrpf(isExpanded ? null : socio.id)}>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="w-0.5 h-0.5 rounded-full bg-border" />
                        <span className="text-[8px] font-black text-primary/60 uppercase tracking-tighter">{socio.vincular.length} {socio.vincular.length === 1 ? 'EMPRESA' : 'EMPRESAS'}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-6">
                ...
                {isExpanded && (
                  <div className="border-t border-border/5 p-4 space-y-4 bg-black/[0.01] dark:bg-white/[0.01] animate-in slide-in-from-top-2 duration-200">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 bg-black/5 dark:bg-white/5 p-4 rounded-xl border border-border/10 shadow-inner">
                      ...
                      <div className="space-y-1">
                        <label className="text-[8px] font-black text-muted-foreground/50 uppercase tracking-widest ml-1">Situação</label>
                        <select value={decl.situacao} onChange={e => updateIRPF(socio.id, "situacao", e.target.value)} className="w-full h-9 px-3 bg-card border border-border/10 rounded-lg text-[10px] font-black uppercase tracking-widest outline-none focus:ring-1 focus:ring-primary/20 cursor-pointer">
                          <option value="pendente">PENDENTE</option>
                          <option value="em_andamento">ANDAMENTO</option>
                          <option value="finalizada">FINALIZADA</option>
                        </select>
                      ...
                      <div className={`space-y-1 ${decl.situacao !== 'finalizada' ? "opacity-20 pointer-events-none" : ""}`}>
                        <label className="text-[8px] font-black text-muted-foreground/50 uppercase tracking-widest ml-1">Quem / Data</label>
                         <div className="flex gap-1 h-9">
                            <select value={decl.quem_transmitiu || ""} onChange={e => updateIRPF(socio.id, "quem_transmitiu", e.target.value)} className="flex-1 bg-card border border-border/10 rounded-lg text-[10px] font-black uppercase tracking-widest outline-none px-2 focus:ring-1 focus:ring-primary/20">
                                <option value="">-</option>
                                {profiles.map(p => <option key={p.id} value={p.nome}>{p.nome}</option>)}
                            </select>
                            <input type="date" value={decl.data_transmissao || ""} onChange={e => updateIRPF(socio.id, "data_transmissao", e.target.value)} className="w-24 bg-card border border-border/10 rounded-lg text-[9px] font-mono outline-none px-1 focus:ring-1 focus:ring-primary/20" />
                        </div>
                      </div>
                      <div className="flex items-end">
                        <button onClick={() => saveIRPF(socio.id)} disabled={saving} className="w-full h-9 inline-flex items-center justify-center gap-2 rounded-lg bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-widest shadow-md hover:bg-primary/90 transition-all disabled:opacity-50">
                          {saving ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Save size={14} /> SALVAR</>}
                        </button>
                      </div>
                    </div>
