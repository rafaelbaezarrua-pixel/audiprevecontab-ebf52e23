...
const PessoalPage: React.FC = () => {
  ...
  const toggleExpand = async (id: string) => {
    ...
    if (!existing.id) {
      ...
      if (prev?.[0]) {
        infoGerais = {
          ...
          possui_vr: prev[0].possui_vr || false,
      ...
      infoGerais = {
        ...
        possui_vr: existing.possui_vr || false,
    ...
    setEditForm(prev => ({
      ...prev, [id]: {
        ...
        vt_status: existing.vt_status || "pendente",
        va_status: existing.va_status || "pendente",
        vr_status: existing.vr_status || "pendente",
        vc_status: existing.vc_status || "pendente",
        inss_status: existing.inss_status || "pendente",
        fgts_status: existing.fgts_status || "pendente",
        inss_data_envio: existing.inss_data_envio || "",
        fgts_data_envio: existing.fgts_data_envio || "",
        vt_data_envio: existing.vt_data_envio || "",
  ...
  const handleSaveAction = async (empresaId: string) => {
    ...
    try {
      await savePessoalRecord({
        ...
        possui_vt: !!form.possui_vt, possui_va: !!form.possui_va, possui_vr: !!form.possui_vr, possui_vc: !!form.possui_vc,
        ...
        inss_data_envio: form.inss_data_envio || null,
        fgts_data_envio: form.fgts_data_envio || null,
        vt_data_envio: form.vt_data_envio || null,
        va_data_envio: form.va_data_envio || null,
        vr_data_envio: form.vr_data_envio || null,
        vc_data_envio: form.vc_data_envio || null,
  ...
  const handleSaveParameters = async (empresaId: string, data: any) => {
    try {
      await savePessoalRecord({
        ...
        possui_vt: !!data.possui_vt, possui_va: !!data.possui_va, possui_vr: !!data.possui_vr, possui_vc: !!data.possui_vc,
      ...
      setEditForm(prev => ({ ...prev, [empresaId]: { ...prev[empresaId], ...data, observacoes: data.observacoes || {} } }));
  ...
  const checkIsDone = (r: Partial<PessoalRecord> | undefined) => {
    if (!r) return false;
    if (!r.dctf_web_gerada) return false;
    if (r.inss_status !== "enviada") return false;
    if (r.fgts_status !== "enviada") return false;
    if (r.possui_vt && r.vt_status !== "enviada") return false;
    if (r.possui_vr && r.vr_status !== "enviada") return false;
    if (r.possui_va && r.va_status !== "enviada") return false;
    if (r.possui_vc && r.vc_status !== "enviada") return false;
    return true;
  };

  const completedCount = React.useMemo(() => {
    return filtered.filter(e => checkIsDone(pessoalData[e.id] as PessoalRecord)).length;
        ...
        <div className="bg-black/[0.03] dark:bg-white/[0.02] border border-border/10 rounded-[2.5rem] shadow-inner p-1 pb-4 relative">
          ...
          <div className="space-y-3 px-1 relative z-10">
            {filtered.map(emp => {
              ...
              const r = pessoalData[emp.id] as Partial<PessoalRecord> | undefined;
              const done = checkIsDone(r);
                      ...
                      <TabsContent value="dados" className="space-y-4 animate-in fade-in slide-in-from-bottom-1 duration-200 outline-none">
                        ...
                        <div className="bg-card p-4 rounded-2xl border border-border/10 shadow-sm space-y-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            {/* DCTF Web */}
                            <div className="space-y-2 p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-border/5 shadow-inner">
                              <h5 className="text-[10px] font-black text-foreground uppercase tracking-widest border-b border-border/10 pb-1">DCTF Web</h5>
                              <div className="grid grid-cols-1 gap-2">
                                <select value={editForm[emp.id]?.dctf_web_gerada ? "sim" : "nao"} onChange={e => updateForm(emp.id, "dctf_web_gerada", e.target.value === "sim")} className="w-full h-9 px-3 rounded-md border border-border/10 bg-card text-[10px] font-black uppercase outline-none focus:ring-1 focus:ring-primary/20 transition-all shadow-sm">
                            ...
                            {/* INSS */}
                            <div className="space-y-2 p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-border/5 shadow-inner">
                              <h5 className="text-[10px] font-black text-foreground uppercase tracking-widest border-b border-border/10 pb-1">INSS</h5>
                              <div className="grid grid-cols-1 gap-2">
                                <select value={editForm[emp.id]?.inss_status || "pendente"} onChange={e => updateForm(emp.id, "inss_status", e.target.value)} className="w-full h-9 px-3 rounded-md border border-border/10 bg-card text-[10px] font-black uppercase outline-none focus:ring-1 focus:ring-primary/20 transition-all shadow-sm">
                            ...
                            {/* FGTS */}
                            <div className="space-y-2 p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-border/5 shadow-inner">
                              <h5 className="text-[10px] font-black text-foreground uppercase tracking-widest border-b border-border/10 pb-1">FGTS</h5>
                              <div className="grid grid-cols-1 gap-2">
                                <select value={editForm[emp.id]?.fgts_status || "pendente"} onChange={e => updateForm(emp.id, "fgts_status", e.target.value)} className="w-full h-9 px-3 rounded-md border border-border/10 bg-card text-[10px] font-black uppercase outline-none focus:ring-1 focus:ring-primary/20 transition-all shadow-sm">
                                  <option value="pendente">PENDENTE</option>
                                  <option value="gerada">GUIA OK</option>
                                  <option value="enviada">ENVIADO</option>
                                </select>
                            ...
                            <div className="space-y-3 pt-3 border-t border-border/5">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                                {editForm[emp.id]?.possui_vt && (
                                  <div className="space-y-2 p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-border/5 shadow-inner">
                                    <h5 className="text-[10px] font-black text-foreground uppercase tracking-widest border-b border-border/10 pb-1">Vale Transporte</h5>
                                    <div className="grid grid-cols-1 gap-2">
                                      <select value={editForm[emp.id]?.vt_status || "pendente"} onChange={e => updateForm(emp.id, "vt_status", e.target.value)} className="w-full h-9 px-3 rounded-md border border-border/10 bg-card text-[10px] font-black uppercase outline-none focus:ring-1 focus:ring-primary/20 transition-all shadow-sm">
                                        <option value="pendente">PENDENTE</option>
                                ...
                                {editForm[emp.id]?.possui_vr && (
                                  <div className="space-y-2 p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-border/5 shadow-inner">
                                    <h5 className="text-[10px] font-black text-foreground uppercase tracking-widest border-b border-border/10 pb-1">Vale Refeição</h5>
                                    <div className="grid grid-cols-1 gap-2">
                                      <select value={editForm[emp.id]?.vr_status || "pendente"} onChange={e => updateForm(emp.id, "vr_status", e.target.value)} className="w-full h-9 px-3 rounded-md border border-border/10 bg-card text-[10px] font-black uppercase outline-none focus:ring-1 focus:ring-primary/20 transition-all shadow-sm">
                                        <option value="pendente">PENDENTE</option>
                                        <option value="gerada">GERADA</option>
                                        <option value="enviada">ENVIADO</option>
                                      </select>
                                ...
                                {editForm[emp.id]?.possui_va && (
                                  <div className="space-y-2 p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-border/5 shadow-inner">
                                    <h5 className="text-[10px] font-black text-foreground uppercase tracking-widest border-b border-border/10 pb-1">Vale Alimentação</h5>
                                    <div className="grid grid-cols-1 gap-2">
                                      <select value={editForm[emp.id]?.va_status || "pendente"} onChange={e => updateForm(emp.id, "va_status", e.target.value)} className="w-full h-9 px-3 rounded-md border border-border/10 bg-card text-[10px] font-black uppercase outline-none focus:ring-1 focus:ring-primary/20 transition-all shadow-sm">
                                        <option value="pendente">PENDENTE</option>
                                        <option value="gerada">GERADA</option>
                                        <option value="enviada">ENVIADO</option>
                                      </select>
                                ...
                                {editForm[emp.id]?.possui_vc && (
                                  <div className="space-y-2 p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-border/5 shadow-inner">
                                    <h5 className="text-[10px] font-black text-foreground uppercase tracking-widest border-b border-border/10 pb-1">Vale Combustível</h5>
                                    <div className="grid grid-cols-1 gap-2">
                                      <select value={editForm[emp.id]?.vc_status || "pendente"} onChange={e => updateForm(emp.id, "vc_status", e.target.value)} className="w-full h-9 px-3 rounded-md border border-border/10 bg-card text-[10px] font-black uppercase outline-none focus:ring-1 focus:ring-primary/20 transition-all shadow-sm">
                                        <option value="pendente">PENDENTE</option>
                                        <option value="gerada">GERADA</option>
                                        <option value="enviada">ENVIADO</option>
                                      </select>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {[
                              ...
                              <div key={b.id} className="flex items-center justify-between p-3 bg-black/5 rounded-xl border border-border/5 group/ben shadow-inner">
                                ...
                                <button onClick={() => updateForm(emp.id, b.id, !editForm[emp.id]?.[b.id])} className={`w-8 h-4 rounded-full relative transition-all duration-300 ${editForm[emp.id]?.[b.id] ? 'bg-primary shadow-[0_0_8px_rgba(var(--primary),0.3)]' : 'bg-black/20'}`}>
                                  <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all duration-300 shadow-sm ${editForm[emp.id]?.[b.id] ? 'left-[18px]' : 'left-0.5'}`} />
                                </button>
                              </div>
                            ))}
                          </div>
